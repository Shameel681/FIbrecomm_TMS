...
<div class="space-y-8" data-aos="fade-up">
    ...
    <div class="bg-white p-8 border border-gray-200 rounded-2xl shadow-sm flex items-center gap-6 relative overflow-hidden">
        ...
        <div class="h-24 w-24 bg-brand-navy rounded-2xl flex items-center justify-center text-white text-4xl font-black shadow-xl transform -rotate-3 group-hover:rotate-0 transition-transform">
            {{ substr($trainee?->name ?? Auth::user()->name, 0, 1) }}
        </div>
        <div class="relative z-10">
            <h2 class="text-3xl font-bold text-brand-navy">{{ $trainee?->name ?? Auth::user()->name }}</h2>
            <div class="flex items-center gap-3 mt-1">
                ...
                <span class="text-gray-500 text-xs font-bold uppercase tracking-widest">ID: #{{ $trainee?->id ?? 'N/A' }}</span>
            </div>
        </div>
    </div>
            ...
            <div class="bg-white p-6 border border-gray-200 rounded-2xl shadow-sm">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-brand-navy font-bold">{{ $trainee?->email ?? Auth::user()->email }}</p>
                    </div>
                    <div>
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-tighter">Internship Duration</label>
                        <p class="text-gray-700 text-sm font-medium">
                            {{ $trainee?->start_date ?? '---' }} — {{ $trainee?->end_date ?? '---' }}
                        </p>
                        
                        {{-- Added: Days Remaining Badge --}}
                        @if(isset($daysLeft))
                        <div class="mt-2 flex items-center gap-2">
                            <span class="px-2 py-0.5 bg-brand-red/10 text-brand-red text-[10px] font-black rounded-full uppercase tracking-tighter">
            ...
            <div class="bg-white p-8 border border-gray-200 rounded-2xl shadow-sm min-h-full">
                ...
                @else
                    <div class="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <p class="text-gray-500 italic">No application data found for email: <strong>{{ $trainee?->email ?? Auth::user()->email }}</strong></p>
                    </div>
                @endif
            </div>
        </div>

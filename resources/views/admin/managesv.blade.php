...
@section('header_title', 'Core Authority: Manage Supervisors')
...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row gap-6 items-end justify-between">
        <div class="bg-white px-8 py-6 rounded-2xl border border-gray-100 shadow-sm border-l-4 border-l-brand-red min-w-[250px]" data-aos="fade-right">
            <h3 class="text-gray-400 font-black text-[10px] uppercase tracking-widest">Total Supervisors</h3>
            <p class="text-brand-navy text-4xl font-black mt-1">{{ $svs->count() }}</p>
        </div>

        <button onclick="document.getElementById('addSvModal').classList.remove('hidden')" 
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden" data-aos="fade-up">
        <div class="p-6 border-b border-gray-50 bg-gray-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-lg font-black text-brand-navy uppercase tracking-tighter">Supervisor Registry</h2>
            ...
            <div class="relative w-full md:w-80">
                ...
                <input type="text" id="svSearchInput" onkeyup="filterSvTable()" 
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse" id="svRegistryTable">
                <thead>
                    <tr class="text-[9px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100">
                        <th class="px-8 py-4">Supervisor Identity</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td class="px-8 py-5 text-center text-[11px] font-bold text-brand-red uppercase">
                            {{-- Corrected to supervisorProfile --}}
                            {{ $sv->supervisorProfile->employee_id ?? 'N/A' }}
                        </td>
                        <td class="px-8 py-5 text-center text-[11px] font-bold text-gray-500 lowercase">
                            {{ $sv->email }}
                        </td>
                        <td class="px-8 py-5 text-center text-[10px] font-bold text-gray-400">{{ $sv->created_at->format('d M Y') }}</td>
                        <td class="px-8 py-5">
                            <div class="flex justify-end gap-2 items-center">
                                {{-- VIEW ACTION --}}
                                <button onclick="openViewModal({{ json_encode($sv->load('supervisorProfile')) }})" title="View Details" class="p-2 hover:bg-blue-50 text-blue-600 rounded-lg transition-colors">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </button>
                                
                                {{-- EDIT ACTION --}}
                                <button onclick="openEditModal({{ json_encode($sv->load('supervisorProfile')) }})" title="Modify Account" class="p-2 hover:bg-gray-100 text-brand-navy rounded-lg transition-colors">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </button>

                                {{-- DELETE ACTION --}}
                                <form action="{{ route('admin.sv.destroy', $sv->id) }}" method="POST" onsubmit="return confirm('CRITICAL: Permanent deletion of Supervisor account. Proceed?')">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="p-20 text-center opacity-20 italic">No Supervisors detected in system.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<div id="addSvModal" class="fixed inset-0 z-[100] hidden bg-brand-navy/90 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-md rounded-2xl overflow-hidden shadow-2xl">
        <div class="bg-brand-navy p-6 text-white flex justify-between items-center">
            <h3 class="font-black uppercase tracking-widest text-sm text-white">Register Supervisor</h3>
            <button onclick="document.getElementById('addSvModal').classList.add('hidden')" class="text-white/50 hover:text-white">&times;</button>
        </div>
        <form action="{{ route('admin.sv.store') }}" method="POST" class="p-8 space-y-5">
            @csrf
            <div class="space-y-1">
                <label class="text-[10px] font-black uppercase text-gray-400 tracking-widest">Employee ID</label>
                <input type="text" name="employee_id" required placeholder="e.g. SV-2024-001" class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl outline-none font-bold text-sm uppercase">
            </div>
            <div class="space-y-1">
...
{{-- MODAL: VIEW SUPERVISOR --}}
<div id="viewSvModal" class="fixed inset-0 z-[100] hidden bg-brand-navy/90 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-md rounded-2xl overflow-hidden shadow-2xl">
        <div class="bg-gray-50 p-6 border-b flex justify-between items-center">
            <h3 class="font-black uppercase tracking-widest text-sm text-brand-navy">Supervisor Profile</h3>
            <button onclick="closeModal('viewSvModal')" class="text-gray-400 hover:text-brand-navy">&times;</button>
        </div>
...
<div id="editSvModal" class="fixed inset-0 z-[100] hidden bg-brand-navy/90 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-md rounded-2xl overflow-hidden shadow-2xl">
        <div class="bg-brand-navy p-6 text-white flex justify-between items-center">
            <h3 class="font-black uppercase tracking-widest text-sm text-white">Modify Supervisor Records</h3>
            <button onclick="closeModal('editSvModal')" class="text-white/50 hover:text-white">&times;</button>
        </div>
        <form id="editSvForm" method="POST" class="p-8 space-y-5">
            @csrf @method('PUT')
            <div class="space-y-1">
                <label class="text-[10px] font-black uppercase text-gray-400 tracking-widest">Employee ID</label>
...
<script>
    ...
    function filterSvTable() {
        ...
        const table = document.getElementById("svRegistryTable");
    ...
    function openViewModal(user) {
        ...
        // Changed to supervisor_profile (snake_case)
        const profile = user.supervisor_profile; 
        document.getElementById('view_employee_id').innerText = profile ? profile.employee_id : 'N/A';
        
        document.getElementById('viewSvModal').classList.remove('hidden');
    }
    ...
    function openEditModal(user) {
        ...
        const profile = user.supervisor_profile;
        if(profile) {
            document.getElementById('edit_employee_id').value = profile.employee_id || '';
        }
        
        document.getElementById('editSvForm').action = "/admin/sv/update/" + user.id;
        document.getElementById('editSvModal').classList.remove('hidden');
    }
